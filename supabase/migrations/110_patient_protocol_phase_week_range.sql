-- 110_patient_protocol_phase_week_range.sql
-- protocol_phases.week_range_label (migratie 054) werd nooit meegekopieerd
-- naar patient_protocol_phases in assign_protocol_to_patient(). Daardoor
-- faalde loadMyProtocolTimeline() (patientProtocolService.ts) stil op een
-- niet-bestaande kolom, en toonde PhaseTimeline nooit iets voor patiënten.
-- Deze migratie voegt de kolom toe, backfilled bestaande fases via
-- source_phase_id, en breidt assign_protocol_to_patient() uit zodat nieuwe
-- toewijzingen het label meteen meekrijgen.

alter table public.patient_protocol_phases
  add column if not exists week_range_label text;

update public.patient_protocol_phases ppp
set week_range_label = pp.week_range_label
from public.protocol_phases pp
where ppp.source_phase_id = pp.id
  and ppp.week_range_label is null;
...
begin
  ...
  loop
    insert into public.patient_protocol_phases
      (patient_protocol_id, source_phase_id, sort_order, name, description, therapist_notes, status, started_at, forbidden_activities, week_range_label)
    values
      (v_patient_protocol_id, v_phase.id, v_phase.sort_order, v_phase.name, v_phase.description, v_phase.therapist_notes,
       ...
       v_phase.forbidden_activities, v_phase.week_range_label)
