  ...
  return (
    <div className="lg:hidden" style={{ background: "#18181a" }}>
      <div className="px-4 py-4 flex items-center gap-2">
        <div className="w-6 h-6 rounded-lg flex items-center justify-center shrink-0 overflow-hidden" style={{ background: "var(--portal-accent, #e8632a)" }}>
          {branding?.logoUrl ? (
      ...
      <div className="flex items-center gap-1 px-3 pb-3 overflow-x-auto">
        {mobileNav.map(({ href, label, icon: Icon }) => {
          ...
          return (
            ...
            >
              <Icon size={13} style={{ color: isActive ? "var(--portal-accent, #e8632a)" : "#52525e" }} />
              {label}
            </Link>
          );
        })}
...
export function PortalLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <PortalGate>
      <div className="flex h-full" style={{ ["--portal-accent" as string]: branding?.primaryColor || "#e8632a" }}>
        <PortalSidebar />
